...
const generateTasks = asyncHandler(async (req, res) => {
  ...
  if (colRes.rows.length)
    throw new ApiError.badRequest("column_id does not belong to this board");
  ...
  for (const s of suggestions) {
    ...
    emitToBoard(req.board.id, "task:created", { task: rows[0] });
  }
  ...
  logActivity({
    ...
    message: `${req.user.name} generated ${created.length} task with AI`,
...
const breakdownTask = asyncHandler(async (req, res) => {
  ...
  const substasks = await ai.breakdownTask(title, description, count);
  res.json({ substasks });
});
