...
const breakdownTask = async (title, description = "", count = 5) => {
  const prompt = `Break the following tasks into ${count} smaller, sequential substasks.
  Task title: "${title}"
  Task details: "${description || "n/a"}"
  
  Return ONLY with a JSON array. Each item:{"title": string, "description": "string (short)", "priority": "low|medium|high|urgent"}
  `;
